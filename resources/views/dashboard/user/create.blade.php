<div class="modal fade" id="createAdModal{{$user->teacher->id}}" aria-hidden="true">
        ...
        <div class="modal-content">
            <form class="form-horizontal" hx-post="{{ route('dashboard.ads.store') }}" hx-encoding='multipart/form-data'>
                ...
                <div class="modal-body">
                    ...
                    <div class="card-body">
                      <div class="d-flex align-items-start align-items-sm-center gap-4">
                          <div hidden><img src="{{asset('assets/img/icons/ad-not-found.jpg')}}" alt="image"
                                  class="d-block rounded" height="120" width="500" id="old-image{{ $user->teacher->id }}" /> </div>
                          <img src="{{asset('assets/img/icons/ad-not-found.jpg')}}" alt="image"
                              class="d-block rounded" height="120" width="500" id="uploaded-image{{ $user->teacher->id }}" />
                      </div>
                      <div class="button-wrapper" style="text-align: center;">
                          <br>
                          <label for="image{{ $user->teacher->id }}" class="btn btn-primary" tabindex="0">
                              <span class="d-none d-sm-block">{{ trans('app.addimage') }}</span>
                              <i class="bx bx-upload d-block d-sm-none"></i>
                              <input class="image-input" type="file" id="image{{$user->teacher->id}}" name="image" hidden
                                  accept="image/png, image/jpeg" />
                          </label>
                          <button type="button" class="btn btn-outline-secondary image-reset" id="reset{{$user->teacher->id}}">
                    ...
                    <input type="hidden" id="teacher_id" name="teacher_id" value="{{$user->teacher->id}}"/>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ trans('ad.teachertype') }}" disabled />
                        <input type="text" class="form-control" id="type" name="type" value="teacher" hidden />
                    </div>
