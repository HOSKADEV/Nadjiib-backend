<div class="modal fade" id="editAdModal{{ $ad->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            <form class="form-horizontal" hx-post="{{ route('dashboard.ads.update', $ad->id) }}"
                ...
                <div class="modal-body">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-start align-items-sm-center gap-4">
                            <div hidden><img
                                    src="{{ empty($ad->image) ? asset('assets/img/icons/ad-not-found.jpg') : url($ad->image) }}"
                                    alt="image" class="d-block rounded" height="120" width="500"
                                    id="old-image{{ $ad->id }}" /> </div>
                            <img src="{{ empty($ad->image) ? asset('assets/img/icons/ad-not-found.jpg') : url($ad->image) }}"
                                alt="image" class="d-block rounded" height="120" width="500"
                                id="uploaded-image{{ $ad->id }}" />
                        </div>
                        <div class="button-wrapper" style="text-align: center;">
                            <br>
                            <label for="image{{ $ad->id }}" class="btn btn-primary" tabindex="0">
                                <span class="d-none d-sm-block">{{ trans('app.addimage') }}</span>
                                <i class="bx bx-upload d-block d-sm-none"></i>
                                <input class="image-input" type="file" id="image{{ $ad->id }}" name="image"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ trans('ad.' . $ad->type . 'type') }}"
                            disabled />
                        <input type="text" class="form-control" id="type" name="type"
                            value="{{ $ad->type }}" hidden />
                    </div>
                    @if ($ad->type == 'url')
                        <div class="mb-3">
